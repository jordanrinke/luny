            ...
            if let Some(parser) = factory.get_parser(path) {
                if let Ok(ast_info) = parser.extract_ast_info(&source, path) {
                    ...
                    let calls: Vec<(String, String)> = ast_info
                        ...
                        .map(|call| {
                            (
                                normalize_separators(&resolve_import_path(&call.target, path, root)),
                                call.method.clone(),
                            )
                        })
                        .collect();
...
fn process_event(
    ...
    for path in &event.paths {
        ...
        if path.strip_prefix(root).map(|p| p.starts_with(".ai")).unwrap_or(false) {
            continue;
        }
...
fn process_pending_changes(
    ...
    for source_path in &to_regenerate {
        match regenerate_single_file(source_path, dep_graph, factory, &threshold_matcher, root, verbose)
        {
            Ok(_) => regenerated += 1,
