...
impl DependencyGraph {
    ...
    pub fn add_file(&mut self, file_path: &str, file_imports: Vec<String>, file_calls: Vec<(String, String)>) {
        let file_path = file_path.to_string();

        // Store forward maps
        self.imports.insert(file_path.clone(), file_imports.clone());
        let call_targets: Vec<String> = file_calls.iter().map(|(target, _)| target.clone()).collect();
    ...
    pub fn update_file(&mut self, file_path: &str, file_imports: Vec<String>, file_calls: Vec<(String, String)>) -> AffectedFiles {
...
mod tests {
    ...
    fn test_remove_file_cleans_maps() {
        ...
        assert!(graph.imports.get("main.ts").is_none());
    ...
    fn test_normalize_separators() {
        assert_eq!(normalize_separators("src\\utils\\file.ts"), "src/utils/file.ts");
        assert_eq!(normalize_separators("src/utils/file.ts"), "src/utils/file.ts");
    }
